...
def create_project(class_id: str, data: CreateProjectRequest, payload: dict = Depends(verify_supabase_token)):
    ...
    client = service_client if service_client else supabase
    enrollment = client.table('class_enrollments').select('id').eq('user_id', user_id).eq('class_id', class_id).execute()
...
def get_class_projects(class_id: str, payload: dict = Depends(verify_supabase_token)):
    if not payload or not payload.get('sub'):
        raise HTTPException(status_code=401, detail="Authentication required")
    ...
    return {"projects": class_projects}

@app.get('/api/projects')
def get_projects(payload: dict = Depends(verify_supabase_token)):
    if not payload or not payload.get('sub'):
        raise HTTPException(status_code=401, detail="Authentication required")
