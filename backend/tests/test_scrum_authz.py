...
def _client_with(member_rows, project_row):
    client = MagicMock()
    members_q = MagicMock()
    members_q.select.return_value.eq.return_value.eq.return_value.limit.return_value.execute.return_value = MagicMock(
        data=member_rows
    )
    projects_q = MagicMock()
    maybe = MagicMock()
    maybe.data = project_row
    projects_q.select.return_value.eq.return_value.maybe_single.return_value.execute.return_value = (
        maybe if project_row else None
    )
    client.table.side_effect = lambda name: {"project_members": members_q, "projects": projects_q}[
        name
    ]
    return client
...
def test_class_instructor_gets_staff(mock_client, _inst, _enr):
    ...
    mock_client.return_value = _client_with(
        [], {"id": PID, "class_id": "c1", "assigned_ta_id": None}
    )
