...
def test_before_cursor_applies_keyset_filter(client, _auth):
    ...
    chain.order.return_value.order.return_value.limit.return_value.execute.return_value = MagicMock(
        data=[]
    )
    list_messages(conversation_id="c1", caller_id="alice", before="2026-07-10T00:00:30+00:00|m29")
...
def test_malformed_cursor_400():
    ...
    with patch("app.messages.controller._require_participant"):
        with pytest.raises(HTTPException) as exc:
            list_messages(conversation_id="c1", caller_id="alice", before="not-a-cursor")
...
def test_list_messages_403_for_non_participant_wired(client):
    ...
    (
        ...
        data={"id": "c1", "type": "dm", "user_a": "alice", "user_b": "bob", "project_id": None}
    )
    (
