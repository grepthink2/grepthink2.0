...
def assign_project_ta(
    project_id: UUID, data: AssignProjectTARequest, user_id: str = Depends(require_user)
):
...
def update_project_meeting(
    project_id: UUID, data: UpdateProjectMeetingRequest, user_id: str = Depends(require_user)
):
...
def set_meeting_cadence(
    class_id: UUID, data: SetMeetingCadenceRequest, user_id: str = Depends(require_user)
):
...
def get_ta_schedule(
    class_id: UUID,
    week: int | None = Query(None, description="Term week number (1..N)"),
    meeting: int | None = Query(
        None, description="Meeting index within the week; omit for the current/next meeting"
    ),
...
def upsert_attendance(
    project_id: UUID, data: UpsertAttendanceRequest, user_id: str = Depends(require_user)
):
...
def mark_all_present(
    project_id: UUID, data: MarkAllPresentRequest, user_id: str = Depends(require_user)
):
    records = controller.mark_all_present(
        project_id, user_id, data.week_number, data.meeting_in_week
    )
