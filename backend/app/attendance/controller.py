...
def _current_term_week(start_date, term: str | None) -> int:
...
def list_class_tas(class_id: UUID, user_id: str, role: str) -> list:
    ...
    try:
        ...
        for sid in student_ids:
            p = pmap.get(sid, {})
            name = f"{p.get('first_name') or ''} {p.get('last_name') or ''}".strip() or p.get(
                "email"
            )
            out.append(
                {
...
def get_ta_schedule(
    ...
    try:
        ...
        cls = (
            ...
            .select("id, created_by, term, start_date, meetings_per_week, meeting_duration_minutes")
            .eq("id", cid)
            .execute()
        )
        if not cls.data:
        ...
        if scope == "mine":
            projects = [
                p
                for p in projects
                if p.get("assigned_ta_id") and str(p["assigned_ta_id"]) == str(user_id)
            ]
        elif scope == "my-team":
            mine = (
        ...
        for p in projects:
            ...
            if ta:
                name = (
                    f"{ta.get('first_name') or ''} {ta.get('last_name') or ''}".strip()
                    or ta.get("email")
                )
                assigned_ta = {
                    "id": ta["id"],
...
def get_team_attendance(
    ...
    try:
        ...
        mrow = _meetings_for_projects(client, [str(project_id)], int(meeting_in_week or 1)).get(
            str(project_id)
        )
        status_map: dict = {}
        if mrow:
...
def _validate_slot(client, class_id: str, week_number: int, meeting_in_week: int) -> None:
    cls = (
        client.table("classes").select("term, meetings_per_week").eq("id", str(class_id)).execute()
    )
...
def upsert_attendance(
    ...
    try:
        ...
        meeting = _get_or_create_meeting(
            ...
            project["class_id"],
            str(project_id),
            int(meeting_in_week or 1),
            marker_id=marker_id,
        )
        record = _upsert_one(
            client, meeting["id"], str(project_id), str(person_id), week_number, status, marker_id
        )
        logger.info(
