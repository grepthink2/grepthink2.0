...
export const api = {
  ...
  // Projects
  createProject: async (classId: string, data: {
    title: string;
    description?: string;
    team_size?: number;
    skills?: string[];
    ...
    return apiRequest<{ project: ApiProject }>(`/api/classes/${classId}/projects`, {
      method: 'POST',
      body: JSON.stringify(data),
    });
  },
  ...
  getProjects: async () => {
    return apiRequest<{ projects: ApiProject[] }>('/api/projects');
  },
};
