...
- A failed request throws `ApiError` with `status` and `detail`; its `message` is the server's detail. A 401 dispatches `auth:unauthorized`, and `AuthProvider` signs out locally.
